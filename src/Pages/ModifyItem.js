...
export default class ModifyItem extends Component {
  ...
  postDataForUpdate = async () => {
    ...
    if (this.state.image instanceof Blob) {
      formData.append("files.image", this.state.image, this.state.image.name);
    ...
    axios({
      ...
      .then((success) => {
        ...
        newObj.apiData = this.state.apiData.map((item) => {
          if (item.id === success.data.id) return success.data;
          else return item;
        });
  ...
  render() {
    ...
    return (
      <div>
        ...
        <SmartTable
          onModify={this.loadRecord}
          content={this.state.apiData}
        ></SmartTable>
      </div>
    );
  }
